-- 0022_dedup_no_merge_rules.sql — Sprint 14 dedup/merge feature
-- (integrator-gap tasket 20260601-110828-76e8).
--
-- Adds a per-workspace dedup_no_merge_rules table so users can mark two
-- records as "distinct, never suggest again" (mirrors Léo's HubSpot
-- multi-email no-merge rule, vault #1060). The table stores canonical pairs
-- (id_a < id_b) per entity_type so a single UNIQUE constraint covers both
-- orderings.
--
-- The provisioning function is updated with step 19; existing workspaces are
-- back-filled via a DO block at the bottom of this file (same pattern as
-- 0015_activities_notes_tasks.sql). The function body is a complete
-- restatement of the 0015 body + this new step — required because
-- CREATE OR REPLACE must supply the full definition.
--
-- References:
--   packages/db/migrations/0015_activities_notes_tasks.sql — prior body
--   docs/adr/ADR-007-audit-log.md — audit catalogue (merge event)
--   apps/api/internal/crm/dedup.go — the backend handlers
...
-- Backfill: create dedup_no_merge_rules in all existing workspaces.
DO $$
DECLARE
  ws RECORD;
BEGIN
