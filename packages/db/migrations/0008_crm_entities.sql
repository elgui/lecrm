-- 0008_crm_entities.sql — add Contact, Company, Deal tables to workspace schemas.
--
-- Sprint 4 feature 1: the three core CRM entity tables. Every remaining v0
-- feature (pipeline Kanban, activity log, RBAC, export, Gmail sync, chatboting
-- connector ADR-011) depends on these tables existing.
--
-- All tables live inside the workspace_<uuid> schema (ADR-001 schema-per-tenant).
-- owner_id references core.users.id conceptually but is NOT a DB FK — cross-schema
-- foreign keys violate tenant isolation. Application enforces referential integrity.
--
-- The provisioning function (core.lecrm_provision_workspace) is extended with
-- steps 8-10 for the three entity tables plus indexes.
--
-- References:
--   docs/sprint-plan.md — Sprint 4, feature 1
--   ADR-001 — schema-per-tenant
--   packages/db/migrations/0006_security_definer_hardening.sql — current function body
