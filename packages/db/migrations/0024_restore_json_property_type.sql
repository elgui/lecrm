-- 0024_restore_json_property_type.sql — restore 'json' to the custom-property
-- type CHECK in the workspace-provisioning function.
--
-- REGRESSION FIX. Migration 0009 added 'json' to the property_type CHECK on
-- per-workspace custom_property_definitions. But the provisioning function is
-- restated in full by every later migration that adds a table (0011, 0012,
-- 0013, 0015, 0022), and each of those restatements copied the pre-0009 DDL
-- — silently dropping 'json'. Consequence: every workspace provisioned after
-- 0011 cannot store json custom properties (the feature 0009 shipped is dead
-- for new tenants). This went unnoticed because the integration test that
-- exercises it (TestMetadata_Set_JSONType_ValidAndInvalid) never ran in CI
-- until the integration suite was wired into the merge gate.
--
-- This migration:
--   1. Restates the function (verbatim from 0022) with 'json' restored in
--      step 6's CHECK — fixes provisioning for NEW workspaces.
--   2. Patches EXISTING workspace schemas via the 0009 constraint-swap block —
--      CREATE TABLE IF NOT EXISTS won't re-add the constraint on its own.
--
-- ⚠️  FUTURE AUTHORS: if you restate this function again, KEEP 'json' in the
--     custom_property_definitions property_type CHECK (step 6). The
--     integration suite now guards this — dropping it turns CI red.
--
-- References:
--   packages/db/migrations/0009_metadata_json_type.sql — original json add
--   packages/db/migrations/0022_dedup_no_merge_rules.sql — prior function body
--   apps/api/internal/metadata/metadata_test.go — TestMetadata_Set_JSONType_*
...
-- Patch existing workspace schemas: drop the (json-less) CHECK and add one
-- that includes 'json'. CREATE TABLE IF NOT EXISTS above will NOT touch tables
-- that already exist, so existing tenants need this explicit ALTER. Same
-- pattern as 0009_metadata_json_type.sql.
DO $migrate$
DECLARE
  v_schema TEXT;
  v_conname TEXT;
