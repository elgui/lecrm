-- 0012_email_suppression.sql — email suppression list per workspace (ADR-003).
--
-- Adds the email_suppression table to each workspace schema. The table
-- is consulted by apps/api/internal/email.Service.Send on every outbound
-- transactional send (ADR-003 §Mitigations item 3, "list hygiene as
-- gating"). Reasons map 1:1 to the four bounce/complaint event types in
-- apps/api/internal/email/brevo:
--
--   hard_bounce   ← brevo `hardBounce`
--   blocked       ← brevo `blocked`
--   complaint     ← brevo `spam`
--   unsubscribed  ← brevo `unsubscribed`
--
-- soft bounces and `delivered` deliberately do NOT suppress per Brevo +
-- RFC 5321 best practice.
--
-- References:
--   docs/adr/ADR-003-email-provider-brevo.md §Decision, §Mitigations
--   apps/api/internal/email/brevo/webhook.go — SuppressionReason()
--   apps/api/internal/email/service.go — Send pre-flight check
...
BEGIN
  ...
  -- Consulted before every outbound transactional send. Reasons set by
  -- the inbound webhook ingestor (apps/api/internal/email/service.go:
  -- IngestEvent → Suppression.Upsert). UNIQUE (email) makes upsert
  -- idempotent across replayed webhook deliveries.
  EXECUTE format($f$
  ...
  -- 14. Email send log — drives the 7-day rolling bounce-rate alarm
  -- (ADR-003 §Mitigations item 4). Each outbound `email.send.requested`
  -- inserts one row; each inbound bounce-like webhook updates it. The
  -- table is intentionally narrow: the audit_log retains the full
  -- record. This table exists only so the rate-check query is a single
  -- index scan rather than a JSONB sweep of audit_log.
  EXECUTE format($f$
