...
**Status `next` (2026-06-14): unparked + decoupled from the first-client gate.** Guillaume dropped the "wait for first paying client" precondition and wants the build train to keep moving, so this runs now as standalone self-research (no external-collaborator wait — see Steps). Runs in a fresh session.
...
3. **Upgrade timing** — when do we move tiers? **Decoupled from any client gate (2026-06-14 decision).** Default trigger: upgrade when v1 sequences development actually begins (i.e. just before `inbound.go` is written), so the dev work has a live inbound endpoint to build against. Capture the concrete trigger you pick.
...
- `docs/adr/ADR-003-email-provider-brevo.md` ends with an addendum dated 2026-XX-XX recording:
  ...
  - Upgrade trigger (e.g., "when v1 sequences dev starts" — NOT gated on a client).
